...
/datum/simcard_application/hacking/proc/ddos_buddies(mob/living/user)
	var/input = tgui_input_list(user, "Who needs a lesson in humility?", "DDoS BUDDIES", GLOB.active_public_simcard_list)
	if(!input)
...
/datum/simcard_application/hacking/proc/ping(mob/living/user, radius = ping_range)
	...
	if(ishuman(user))
		var/mob/living/carbon/human/buddy = user
		if(buddy.dna?.species?.id == SPECIES_INBORN)
