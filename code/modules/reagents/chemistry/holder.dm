...
/datum/reagents/proc/metabolize_reagent(mob/living/carbon/owner, datum/reagent/reagent, delta_time, times_fired, can_overdose = FALSE, liverless = FALSE)
	...
	if(ishuman(owner))
		...
		// We'll only process if the species fulfills at least one flag
		if(!(reagent.process_flags & human_owner.dna?.species?.reagent_processing))
			reagent.holder.remove_reagent(reagent.type, reagent.metabolization_rate)
			return FALSE
	// Don't process synthetic reagents unless we are robotic, if we ain't human
	...
	if(owner && reagent)
		...
		if(can_overdose && !HAS_TRAIT(owner, TRAIT_OVERDOSE_IMMUNE))
